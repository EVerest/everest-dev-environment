...
During the *cmake* step **edm** creates a *release.json* file that is located in your build directory, for example *build/release.json*.

This autogenerated json file contains all dependencies and their versions used in the everest-core build as well as some additional metadata.

There are a few environment variables that can be set to change the contents of this file:
```bash
export EVEREST_UPDATE_CHANNEL="stable" # sets the "channel" property in the json
export EVEREST_METADATA_INCLUDE_URL="yes" # controls if the url of a dependency is included, this defaults to "no"
export EVEREST_METADATA_INCLUDE_ALL="no" # controls if every entry in the metadata.yaml is included, defaults to "no"
export EVEREST_METADATA_FILE="/path/to/a/metadata.yaml" # sets a local metadata.yaml, if this is not set it is automatically fetched from https://raw.githubusercontent.com/EVerest/everest-dev-environment/main/everest-metadata.yaml
```

An excerpt from a release.json file:
```json
{
    "channel": "stable",
    "datetime": "2024-02-07T17:18:19.062068Z",
    "version": "2024.1.0",
    "components": [
        {
            "name": "everest-core",
            ...
            "license": "Apache-2.0",
            "url": "https://github.com/EVerest/everest-core.git",
            "dependent": []
        },
        {
            ...
            "license": "Apache-2.0",
            "url": "https://github.com/EVerest/everest-framework.git",
            "dependent": [
                "everest-core"
            ]
        }
